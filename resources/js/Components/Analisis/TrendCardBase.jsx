...
import { RefreshCw, Loader2, Sparkles, X, AlertTriangle, Info } from 'lucide-react';
    ...
    return (
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs relative">
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                {sudahDianalisis && canRegenerasi && !isTidakBerlaku && (
                    <button
            ...
            {isTidakBerlaku ? (
                // Poin 1a: periode pertama, tidak ada data pembanding -> bukan error
                <div className="bg-slate-50/70 border border-slate-200 rounded-lg p-4 flex gap-2 items-start text-slate-500">
                    <Info className="w-4 h-4 shrink-0 mt-0.5" />
                    <p className="text-xs">
                        Tidak berlaku untuk periode pertama perusahaan ini — belum ada periode sebelumnya untuk dibandingkan.
                    </p>
                </div>
            ) : dataKurang ? (
                ...
                <>
                    ...
                    {isLoading ? (
                        <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-3 mb-4 flex items-center gap-2 text-blue-600">
                            <Loader2 className="w-3.5 h-3.5 animate-spin" />
                            <span className="text-xs">Sedang diproses AI...</span>
                        </div>
                    ) : status === 'gagal' ? (
                        <div className="bg-red-50/70 border border-red-200 rounded-lg p-3 mb-4 text-red-700">
                ...
                <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/60 backdrop-blur-sm rounded-xl p-4">
                    <div className="bg-white border border-slate-200 shadow-xl rounded-xl p-5 w-full">
                        ...
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setIsPromptModalOpen(false)} className="px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100 rounded-lg" disabled={isLoading}>
                                Batal
                            </button>
