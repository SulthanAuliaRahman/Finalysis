...
export default function Edit({ user, perusahaanList }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        name: user.name ?? "",
        email: user.email ?? "",
        password: "", // Kosong secara default, hanya diisi jika ingin diubah
        role: user.role ?? "",
        perusahaan_id: user.perusahaan_id ?? "",
        is_active: user.is_active ? 1 : 0, // 1 = Aktif, 0 = Nonaktif
    });
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-4">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs space-y-6">
                <div>
                    <h2 className="text-lg font-bold text-slate-900">Perbarui Data Pengguna</h2>
                    <p className="text-xs text-slate-500 mt-0.5">Ubah informasi akun dan status akses pengguna.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {/* Kata Sandi (Opsional saat Edit) */}
                    <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-semibold text-slate-700" htmlFor="password">
                            Kata Sandi Baru <span className="text-slate-400 font-normal">(Kosongkan jika tidak ingin diubah)</span>
                        </label>
                        <input
                            id="password"
                            type="password"
...
Edit.layout = page => <AppLayout title="Edit Pengguna" children={page} />;
